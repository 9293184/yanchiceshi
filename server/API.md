...
| 变量名 | 说明 |
|--------|------|
| `VITE_COMMONSTACK_API_KEY` | CommonStack API Key |
| `VITE_MOONSHOT_API_KEY` | Moonshot API Key |
| `VITE_QINIU_API_KEY` | 七牛云 API Key |
| `VITE_ZHIPU_API_KEY` | 智谱 API Key |
| `VITE_SILICONFLOW_API_KEY` | 硅基流动 API Key |
| `VITE_STEPFUN_API_KEY` | 阶跃星辰 API Key |
| `PORT` | 端口号（Railway 自动分配，一般不需要手动设置） |
